...
*  Copyright notice
*
*  (c) 2005 Michiel Roos (typo3@monosock.org)
*  All rights reserved
*
 ...
 * $Id$
 *
 * @author	Michiel Roos <typo3@monosock.org>
 *
 */
/**
 ...
 *
 *
 *   67: class tx_wizardcrpagetree_webfunc extends t3lib_extobjbase
 *   75:     function main()
 *  199:     function compressArray($data)
 *  219:     function getArray($data, $oldLevel = 0, $ic)
 *  272:     function reverseArray($data)
 *  294:     function filterComments($data)
 *  331:     function displayCreatForm()
 *  370:     function getIndentationChar()
 *  393:     function getSeparationChar()
 *  419:     function getExtraFields()
 *  435:     function helpBubble()
 *
 * TOTAL FUNCTIONS: 10
    ...
    // create new pages here?
      ...
      if (t3lib_div::_POST('newPageTree') === 'submit') {
        ...
        if (count($data)) {
          ...
          if ($data) {
            ...
            while(list($k,$line)=each($data))	{
              if (trim($line))	{
                ...
                elseif ($level < $oldLevel) {
                  $currentPid = 'NEW'.$parentPid[$level-1];
                }
                ...
                $pageTree['pages']['NEW'.$pageIndex]['sorting']=$pageIndex;
          ...
          $theCode.= '<b>'.$LANG->getLL('wiz_newPageTree_currentMenu').':</b><br /><br />'.implode('<br />',$lines);
        }
      } else {
